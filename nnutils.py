...
# 修改：移除 create_var 函数中的 .cuda() 调用
def create_var(tensor, requires_grad=None):
    if requires_grad is None:
        return Variable(tensor) # 移除 .cuda()
    else:
        return Variable(tensor, requires_grad=requires_grad) # 移除 .cuda()

# 修改：移除 create_var_gpu 函数中的 .cuda() 调用
