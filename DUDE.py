...
def write_csv(affinities,smiles,out_path):
    csv_frame = pd.DataFrame([ { 'Affinity':affinity,'SMILES':smi, } for affinity,smi in zip(affinities,smiles) ])
...
if __name__ == "__main__":
    ...
    # 修改：将 'cuda' 改为 'cpu'
    PLANET = PLANET(feature_dims,nheads,key_dims,value_dims,pro_update_inters,lig_update_iters,pro_lig_update_iters,'cpu') # 'cuda'
    # 修改：加载 state_dict 时指定 map_location
    PLANET.load_state_dict(torch.load(args.PLANET_file, map_location='cpu')) # 'cpu'
    ...
    with torch.no_grad():
        for folder in os.listdir(args.DUDE):
            ...
            # torch.cuda.empty_cache() # 移除或注释掉这行，因为不用 CUDA
    ...
    csv_frame = pd.DataFrame([ { 'Target Name':target_name,'Active Mean':active_mean,'Active Median':active_median,
     'Decoy Mean':decoy_mean,'Decoy Median':decoy_median,'Delta Mean':active_mean-decoy_mean,'Delta Median':active_mean-decoy_median,
     'EF_0.5%':x,'EF_1%':y,'EF_2%':z,'EF_5%':w,'ROC_AUC Score':auc_score,'ROC_PR Score':auc_pr } for target_name,active_mean,active_median,decoy_mean,decoy_median,x,y,z,w,auc_score in zip(target_names,active_means,active_medians,decoy_means,decoy_medians,EF_05,EF_1,EF_2,EF_5,auc_scores,auc_prs) ])
